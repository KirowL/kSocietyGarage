local Statistics = {
    Background = { Dictionary = "commonmenu", Texture = "gradient_bgd", Y = 134, Width = 431, Height = 42 },
    Text = {
        Left = { X = -40, Y = 143, Scale = 0.35 },
    },
    Bar = { Right = 8, Y = 154, Width = 200, Height = 10, OffsetRatio = 0.5 },
    Divider = {
        [1] = { X = 200, Y = 154, Width = 2, Height = 10 },
        [2] = { X = 200, Y = 154, Width = 2, Height = 10 },
        [3] = { X = 200, Y = 154, Width = 2, Height = 10 },
        [4] = { X = 200, Y = 154, Width = 2, Height = 10 },
        [5] = { X = 200, Y = 154, Width = 2, Height = 10 },
    }
}
...
function RageUI.StatisticPanel(Percent, Text, Index)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (Index == nil or (CurrentMenu.Index == Index)) then
            ...
            RenderRectangle(CurrentMenu.X + 520, 10 + Statistics.Background.Y  + (RageUI.StatisticPanelCount * 42), Statistics.Background.Width + CurrentMenu.WidthOffset, Statistics.Background.Height, 0, 0, 0, 170)
            RenderText(Text or "", CurrentMenu.X + 528.0, 10 + (RageUI.StatisticPanelCount * 40) +  Statistics.Text.Left.Y , 0, Statistics.Text.Left.Scale, 245, 245, 245, 255, 0)
            RenderRectangle(CurrentMenu.X + 520 + RageUI.Settings.Items.Title.Background.Width - BarWidth - Statistics.Bar.Right + CurrentMenu.WidthOffset, 10 +(RageUI.StatisticPanelCount * 40)  + Statistics.Bar.Y  , BarWidth, Statistics.Bar.Height, 87, 87, 87, 255)
            RenderRectangle(CurrentMenu.X + 520 + RageUI.Settings.Items.Title.Background.Width - BarWidth - Statistics.Bar.Right + CurrentMenu.WidthOffset, 10 +(RageUI.StatisticPanelCount * 40)  + Statistics.Bar.Y  , Percent * BarWidth, Statistics.Bar.Height, 255, 255, 255, 255)
            for i = 1, #Statistics.Divider, 1 do
                RenderRectangle((CurrentMenu.X + 520 + RageUI.Settings.Items.Title.Background.Width - BarWidth - Statistics.Bar.Right) + i * ((BarWidth - (#Statistics.Divider / Statistics.Divider[i].Width)) / (#Statistics.Divider + 1)) + CurrentMenu.WidthOffset, 10 +(RageUI.StatisticPanelCount * 40)+ Statistics.Divider[i].Y  , Statistics.Divider[i].Width, Statistics.Divider[i].Height, 0, 0, 0, 255)
            end
